...
pushes, no force-pushes), at least 1 approving review, and a green "Build,
vet & test" status check (`ci.yml`'s `test` job — its `name:` must keep
matching that exact string, since the ruleset names it literally, not by
job ID). A companion ruleset (`release-tag-immutability`) makes every
...
gh pr create --fill
# wait for CI to go green on the PR, get it reviewed and approved
gh pr merge --squash   # or --merge/--rebase; any of the three is allowed
```
Since no one can self-approve their own PR, landing a change now always
needs a second person (or another reviewing session) to actually approve
it — budget for that instead of assuming a same-session merge.

**Cutting a release** is the same PR flow, automated end to end except for
that same required human approval:
1. `gh workflow run prepare-release.yml -f version=X.Y.Z` (no leading `v`).
   This generates a changelog (the same "generate release notes" GitHub API
   call `release.yml`'s publish step uses), prepends a dated section to
   ...
   workflow every time.
2. Review the PR like any other (the CI run on it is the same "Build, vet &
   test" job as always — a real, if usually uneventful, check that nothing
   about `main` is broken right before cutting a release from it), then
