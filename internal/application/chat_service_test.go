...
func TestChatService_EmptyHistory(t *testing.T) {
	svc := NewChatService(&fakeChatEndpointStore{}, &fakeChatCompleter{}, nil, nil, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_NotConfigured(t *testing.T) {
	...
	svc := NewChatService(endpoints, &fakeChatCompleter{}, nil, nil, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_MaxContextTokens_Zero_NoTrimming(t *testing.T) {
	...
	svc := NewChatService(endpoints, completer, nil, nil, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_MaxContextTokens_KeepsToolPromptSystemMessageIntact(t *testing.T) {
	...
	svc := NewChatService(endpoints, completer, servers, &fakeMCPToolProvider{}, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_Agent_InjectsSystemPromptBetweenUserPromptAndServerPrompts(t *testing.T) {
	...
	svc := NewChatService(endpoints, completer, servers, &fakeMCPToolProvider{}, agents, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_Agent_PerQuestionOverridesEndpointDefault(t *testing.T) {
	...
	svc := NewChatService(endpoints, completer, nil, nil, agents, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_Agent_ScopesActiveMCPServersToAllowedList(t *testing.T) {
	...
	provider := &fakeMCPToolProvider{}
	svc := NewChatService(endpoints, completer, servers, provider, agents, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_UserMCPServers_MergedRegardlessOfAgentScope(t *testing.T) {
	...
	provider := &fakeMCPToolProvider{}
	svc := NewChatService(endpoints, completer, servers, provider, agents, userServers, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_UserMCPServers_TaggedSelfService(t *testing.T) {
	...
	provider := &fakeMCPToolProvider{}
	svc := NewChatService(endpoints, completer, globalServers, provider, nil, userServers, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_UserMCPServers_StdioTransportRejected(t *testing.T) {
	...
	provider := &fakeMCPToolProvider{}
	svc := NewChatService(endpoints, completer, nil, provider, nil, userServers, VisionConfig{Settings: nil, InternalAPIKey: ""})
...
func TestChatService_UserMCPServers_EmptyUserIDOrNilStoreAddsNothing(t *testing.T) {
	...
	t.Run("nil store", func(t *testing.T) {
		provider := &fakeMCPToolProvider{}
		svc := NewChatService(endpoints, completer, nil, provider, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
		history := []domain.ChatMessage{{Role: domain.ChatRoleUser, Content: "hi"}}
		if _, err := svc.Chat(context.Background(), history, ChatOptions{UserID: "alice"}); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
// fakeChatVisionStore is a minimal ports.ChatVisionStore fake.
...
func (f *fakeChatVisionStore) GetChatVisionSettings(context.Context) (domain.ChatVisionSettings, error) {
	if f.err != nil {
		return domain.ChatVisionSettings{}, f.err
	}
	return f.settings, nil
}

func (f *fakeChatVisionStore) SetChatVisionSettings(context.Context, domain.ChatVisionSettings) error {
	return nil
}

// TestChatService_MCPEnv_CarriesVisionSimilarityConfigWhenEnabled proves
// SimilarityEnabled reaches Open's env map as VISION_SIMILARITY_ENABLED/
// VISION_SIMILARITY_PROVIDER_ID.
func TestChatService_MCPEnv_CarriesVisionSimilarityConfigWhenEnabled(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	vision := &fakeChatVisionStore{settings: domain.ChatVisionSettings{SimilarityEnabled: true, SimilarityProviderID: "h200_gte_qwen2"}}
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: vision, InternalAPIKey: ""})

	history := []domain.ChatMessage{{Role: domain.ChatRoleUser, Content: "hi"}}
	if _, err := svc.Chat(context.Background(), history, ChatOptions{}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got := provider.openedEnv["VISION_SIMILARITY_ENABLED"]; got != "true" {
		t.Errorf("expected VISION_SIMILARITY_ENABLED=true, got %q (env=%v)", got, provider.openedEnv)
	}
	if got := provider.openedEnv["VISION_SIMILARITY_PROVIDER_ID"]; got != "h200_gte_qwen2" {
		t.Errorf("expected VISION_SIMILARITY_PROVIDER_ID, got %q (env=%v)", got, provider.openedEnv)
	}
}

// TestChatService_MCPEnv_OmitsVisionSimilarityConfigWhenDisabled proves a
// disabled (or unconfigured) Similarity leaves both vars out entirely.
func TestChatService_MCPEnv_OmitsVisionSimilarityConfigWhenDisabled(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	vision := &fakeChatVisionStore{settings: domain.ChatVisionSettings{SimilarityEnabled: false}}
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: vision, InternalAPIKey: ""})
	...
	if _, ok := provider.openedEnv["VISION_SIMILARITY_ENABLED"]; ok {
		t.Errorf("expected VISION_SIMILARITY_ENABLED omitted when disabled, got env=%v", provider.openedEnv)
	}
}

// TestChatService_MCPEnv_CarriesVisionCaptionConfigWhenEnabled proves
// CaptionEnabled reaches Open's env map as VISION_CAPTION_ENABLED plus its
// base URL/API key/model.
func TestChatService_MCPEnv_CarriesVisionCaptionConfigWhenEnabled(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	vision := &fakeChatVisionStore{settings: domain.ChatVisionSettings{
		CaptionEnabled: true, CaptionBaseURL: "http://vl.example/v1", CaptionAPIKey: "sk-vl", CaptionModel: "vl-chat",
	}}
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: vision, InternalAPIKey: ""})
	...
	if got := provider.openedEnv["VISION_CAPTION_ENABLED"]; got != "true" {
		t.Errorf("expected VISION_CAPTION_ENABLED=true, got %q", got)
	}
	if got := provider.openedEnv["VISION_CAPTION_BASE_URL"]; got != "http://vl.example/v1" {
		t.Errorf("expected VISION_CAPTION_BASE_URL, got %q", got)
	}
	if got := provider.openedEnv["VISION_CAPTION_API_KEY"]; got != "sk-vl" {
		t.Errorf("expected VISION_CAPTION_API_KEY, got %q", got)
	}
	if got := provider.openedEnv["VISION_CAPTION_MODEL"]; got != "vl-chat" {
		t.Errorf("expected VISION_CAPTION_MODEL, got %q", got)
	}
}

// TestChatService_MCPEnv_OmitsVisionCaptionConfigWhenDisabled mirrors the
// Similarity-disabled case above for Caption.
func TestChatService_MCPEnv_OmitsVisionCaptionConfigWhenDisabled(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	vision := &fakeChatVisionStore{settings: domain.ChatVisionSettings{CaptionEnabled: false}}
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: vision, InternalAPIKey: ""})
	...
	if _, ok := provider.openedEnv["VISION_CAPTION_ENABLED"]; ok {
		t.Errorf("expected VISION_CAPTION_ENABLED omitted when disabled, got env=%v", provider.openedEnv)
	}
}

// TestChatService_MCPEnv_NilVisionSettingsStoreIsANoop proves a nil
// visionSettings (the pre-existing wiring, before this feature existed)
// never adds any VISION_* var and never panics.
func TestChatService_MCPEnv_NilVisionSettingsStoreIsANoop(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: ""})
	...
	for _, key := range []string{"VISION_SIMILARITY_ENABLED", "VISION_CAPTION_ENABLED", "CHAT_VISION_INTERNAL_API_KEY"} {
		if _, ok := provider.openedEnv[key]; ok {
			t.Errorf("expected %s omitted with a nil vision store, got env=%v", key, provider.openedEnv)
		}
	}
}

// TestChatService_MCPEnv_VisionSettingsStoreErrorIsANoop proves a store
// error (e.g. DB unavailable) degrades to no VISION_* vars, never fails
// the turn.
func TestChatService_MCPEnv_VisionSettingsStoreErrorIsANoop(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	vision := &fakeChatVisionStore{err: errors.New("db unavailable")}
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: vision, InternalAPIKey: ""})
	...
	if _, ok := provider.openedEnv["VISION_SIMILARITY_ENABLED"]; ok {
		t.Errorf("expected no vision env vars on a store error, got env=%v", provider.openedEnv)
	}
}

// TestChatService_MCPEnv_CarriesInternalVisionAPIKey proves
// internalVisionAPIKey always reaches Open's env map as
// CHAT_VISION_INTERNAL_API_KEY, independent of vision settings themselves.
func TestChatService_MCPEnv_CarriesInternalVisionAPIKey(t *testing.T) {
	endpoints := &fakeChatEndpointStore{endpoint: domain.ChatEndpoint{Enabled: true}}
	completer, servers, provider := mcpEnvTestFixtures()
	svc := NewChatService(endpoints, completer, servers, provider, nil, nil, VisionConfig{Settings: nil, InternalAPIKey: "internal-secret"})

	history := []domain.ChatMessage{{Role: domain.ChatRoleUser, Content: "hi"}}
	if _, err := svc.Chat(context.Background(), history, ChatOptions{}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got := provider.openedEnv["CHAT_VISION_INTERNAL_API_KEY"]; got != "internal-secret" {
		t.Errorf("expected CHAT_VISION_INTERNAL_API_KEY, got %q", got)
	}
}
