...
// endpoint. When the endpoint has RAGEnabled, the last user message is
// used as a search query against s.search, and any results found are
// woven in as a system message ahead of the rest of history -- a search
// error at this stage is treated as best-effort (the plain history is
// still sent, unaugmented) rather than failing the whole call, since RAG
// context is an enhancement, not a requirement, of answering.
func (s *ChatService) Chat(ctx context.Context, history []domain.ChatMessage) (ChatResult, error) {
	if len(history) == 0 {
