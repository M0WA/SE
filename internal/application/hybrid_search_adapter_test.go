...
func TestHybridAsSearchService_MapsFinalScoreToSearchResult(t *testing.T) {
	...
	svc := application.NewHybridAsSearchService(repo, map[string]ports.EmbeddingProvider{domain.EmbeddingProviderHash: embedder}, domain.NewTuningSettings(0.5, domain.DefaultBM25K1, domain.DefaultBM25B), nil, nil, domain.NewCorpusStatsCache(1, 10), nil)
...
func TestHybridAsSearchService_PropagatesError(t *testing.T) {
	svc := application.NewHybridAsSearchService(&fakeSQLRepo{}, map[string]ports.EmbeddingProvider{domain.EmbeddingProviderHash: &fakeEmbedder{}}, domain.NewTuningSettings(0.5, domain.DefaultBM25K1, domain.DefaultBM25B), nil, nil, nil, nil)
...
func TestHybridAsSearchService_PropagatesRepoError(t *testing.T) {
	svc := application.NewHybridAsSearchService(&erroringSQLRepo{}, map[string]ports.EmbeddingProvider{domain.EmbeddingProviderHash: &fakeEmbedder{}}, domain.NewTuningSettings(0.5, domain.DefaultBM25K1, domain.DefaultBM25B), nil, nil, nil, nil)
