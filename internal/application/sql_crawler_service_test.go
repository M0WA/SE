...
type recordingSQLRepo struct {
	fakeSQLRepo
	// mu guards saved/embeddings against any future test that calls
	// SaveDocument from more than one goroutine at once; every test here
	// today uses this repo from a single goroutine.
	mu         sync.Mutex
