...
// ListFiles lists ownerUserID's files, most recently uploaded first --
// metadata only (no data column), so listing stays cheap regardless of
// file size.
...
// SaveFile inserts a new file owned by ownerUserID, attached to chatID,
// minting its ID (randomFileID) and CreatedAt itself -- the caller never picks either.
...
// exists -- scoping by owner, not just id, keeps one user from downloading
// another's file even if they guessed its ID.
...
// nullableString returns a sql.NullString that's actually NULL for empty s
// -- used for uploaded_files.chat_id, whose FK must see SQL NULL (never
// "", which no chats.id matches) for a file with no chat. Mirrors
// nullableTimeString's convention for a different type.
...
// randomFileID returns a 16-byte hex-encoded random token (32 chars,
// fitting mysql's uploaded_files.id VARCHAR(32)) -- unguessable like a
// session token, since it appears directly in a download URL (see
// dialect.go's uploaded_files comment).
func randomFileID() string {
	...
	if _, err := rand.Read(b); err != nil {
		...
		// file upload (same tolerance dockersandbox.randomHex applies).
