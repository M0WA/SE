...
# One image, four binaries -- run the same way as the systemd services
...
# searchengine-mcp-web is never run directly -- search-server/admin-server
# spawn it themselves as a stdio subprocess (see internal/adapters/mcpclient)
# whenever an admin configures an MCPServer row pointing at it.
