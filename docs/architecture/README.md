...
| Chat vision settings (`chat_vision.go`) | `ChatVisionSettings` -- independent similarity-search (references an existing `EmbeddingHTTPEndpoint` by ID) and captioning (its own full endpoint config) toggles for an image attached to a chat turn, deliberately decoupled from both `EmbeddingHTTPEndpoint`/`EmbeddingSearchWeights` and `ChatEndpoint`. |
...
| `ImageEmbedder` | Optional capability an `EmbeddingProvider` may additionally implement -- image-to-vector conversion (a chat-completions-style request against a vision-language model's embeddings endpoint), checked via type assertion. | `httpembed` (not `hashembed`) |
...
| `PageRankRepository`, `ContentDedupRepository`, `EmbeddingRepository`, `SemanticMatcher` | Narrow slices of `SQLRepository`, scoped to exactly what each background job (or, for `SemanticMatcher`, the vision-similarity endpoint) needs. | `sqlrepo` |
...
| `GPUModeController` | Calls `cmd/gpu-control`'s own small HTTP API (`GET/POST /gpu/api/mode`, `POST /gpu/api/heartbeat`) over the private LAN, given `domain.GPUModeSettings` on every call (same "config per call" convention as `ChatCompleter`, since it's DB-backed and can change between calls). | `httpgpumode` |
...
| `ChatService` | Orchestrates one chat turn: loads config, resolves and injects the active agent (endpoint default or per-question override), merges the caller's own MCP servers into the global catalog (always, never narrowed by the agent's scope), opens an MCP session (passing `cmd/mcp-vision`'s configuration as env vars, read live from `ChatVisionStore` every turn), trims history, delegates completion, and runs any tool calls the model makes. | `ChatEndpointStore`, `ChatCompleter`, `MCPServerStore`, `MCPToolProvider`, `AgentStore`, `UserMCPServerStore`, `ChatVisionStore` |
| `GPUModeService` | Backs `GET/POST /vision/api/mode`, `POST /vision/api/heartbeat`, and `restapi.Handler.handleChat`'s own availability check (consults a cheap in-memory cache, refreshed on a background poll -- never a live network call per chat turn). Enforces the security gate beyond the admin master switch (`GPUModeSettings.Enabled`) and session auth (both `restapi`'s own job): a global minimum dwell between accepted switches and a per-account rate limit, on top of whatever `GPUModeController` itself reports (e.g. a 409 conflict when a switch to a different target is already in flight). | `GPUModeStore`, `GPUModeController` |
...
| `netguard` | Shared SSRF guard (custom `DialContext`), with two policies: a strict one (`AllowedIP`) blocking every private/reserved range, used by every outbound crawler fetch; and a permissive one (`AllowedConfiguredEndpointIP`) for admin-configured endpoints (`httpembed`/`httpchat`'s `BaseURL`/`TokenizeURL`, `httpgpumode`'s `ControlBaseURL`, and `restapi`'s Document-upload S3 import's `endpoint`) that only blocks link-local/multicast/unspecified addresses, since a self-hosted embeddings/chat backend (or an S3-compatible endpoint) legitimately lives on a private network or loopback. |
...
| `httpgpumode` | Calls `cmd/gpu-control`'s HTTP API (`ControlBaseURL`/`ControlAPIKey` from `domain.GPUModeSettings`, sent as `X-Internal-Token`); outbound calls routed through `netguard`'s configured-endpoint policy, same as `httpchat`/`httpembed`. |
...
| `dockersandbox` | Runs untrusted Python/Go snippets in a locked-down Docker container via the `docker` CLI -- memory/CPU/process/wall-clock limits, dropped capabilities, read-only rootfs, no network unless configured. `RunOptions.Files` additionally mounts named input files into the same read-only bind mount as the code itself, independent of network access. `RunOptions.SystemPackages` (apt/apk) runs in a *separate* short-lived container first (Docker's ordinary default capabilities, writable root -- but never running the model's own code, only the fixed install command), commits it to a throwaway image, then runs the actual snippet from that image under the sandbox's completely normal lockdown -- installing packages and running untrusted code never share a container. Used only by `cmd/mcp-sandbox`; not wired through any port, since that MCP binary is the entire integration surface. |

## Binaries

- **`cmd/search`** — public, internet-facing. Serves the index/search page, `/search` JSON API, chat endpoints (`POST /chat`, `GET /agents`), `/session` (nav role lookup), `/account`/`/account/api` (self-service password/chat-prompt), `/account/mcp-servers`/`/account/api/mcp-servers...` (self-service `http`-only MCP CRUD, merged into every chat turn), `/account/files`/`/account/api/files...` (uploaded-file storage, also reachable via a short-lived bearer token for `cmd/mcp-files`), `/search/api/vision-similarity` (internal-only, `X-Internal-API-Key`-gated by `CHAT_VISION_INTERNAL_API_KEY` -- embeds an image via a configured `EmbeddingProvider` and runs a pgvector ANN search with it, backing `cmd/mcp-vision`'s `vision_similarity` tool), and `/vision/api/mode`/`/vision/api/heartbeat` (GPU mode (Vision) -- image/video generation, requires a signed-in session, backed by `GPUModeService`/`httpgpumode` calling `cmd/gpu-control` on the separate GPU host; absent as 404 when not admin-enabled). The only one of the three exposed to the public internet. Talks to the shared DB via `sqlrepo`, never to the other two binaries directly.
- **`cmd/admin`** — internal, reached only via nginx's `/admin`, `/login`, `/logout` prefixes. Hosts every `/admin/api/*` endpoint (settings, embedding endpoints, PageRank, content-dedup, sessions/auth, diagnostics, scheduled crawls, chat endpoints, chat vision settings). Never fetches pages or touches robots.txt/documents itself -- only starts/polls crawl jobs on crawl-server via `crawlclient` (default `CRAWL_SERVER_URL=http://127.0.0.1:8082`, optional `X-Internal-Token`).
...
- **`cmd/mcp-sandbox`** — first-party MCP server exposing `run_python`/`run_go`, running a model-supplied snippet in a locked-down Docker container via `dockersandbox`. Same spawn model. Resource limits and network access are admin-configured via the `MCPServer` row's `Args` (e.g. `-network`, `-memory=1g`) or a matching `SE_SANDBOX_*` env var (an explicit Args flag wins), never per-call. Requires `docker` CLI access for the `searchengine` service user -- a real privilege elevation (Docker access is host-root-equivalent); see `docs/manual/installation.md` step 16 before enabling. Both tools also take an optional model-supplied `file_ids`: fetched from search-server's own `/account/api/files/{id}` using the same per-turn `SE_FILES_API_TOKEN` bearer token `cmd/mcp-files` uses (mcpclient passes it to every spawned server this turn, not just `mcp-files`), then written into the sandbox's own working directory -- a filesystem mount, independent of `-network`, so a generated script can process any file type (PDF, docx, image, ...) as an ordinary local file. With both `-network` and `-system-packages` set, also takes a model-supplied `system_packages` (apt/apk package names) -- see `dockersandbox`'s own entry above for why that provisions a separate throwaway image rather than installing directly into the code-running container.
...
- **`cmd/mcp-vision`** — first-party MCP server exposing `vision_similarity` (embeds an attached image and vector-searches this instance's own index with it, via `cmd/search`'s `/search/api/vision-similarity`) and `vision_caption` (describes an attached image, or answers a question about it, by calling a configured vision-language chat-completions endpoint directly). Same spawn model as `cmd/mcp-files` for reading the attached image (`SE_FILES_API_TOKEN` against `/account/api/files`); holds no DB connection or embedding-endpoint credential of its own either -- `vision_similarity`'s actual embed+search happens inside `cmd/search`, reached only via the separate `CHAT_VISION_INTERNAL_API_KEY`. Both capabilities are independently admin-configured (`domain.ChatVisionSettings`) and independent of search's own embedding endpoints, even where Similarity ends up referencing the same underlying model. Distinct from GPU mode (Vision) below, which is image/video *generation*, not understanding.
- **`cmd/gpu-control`** — root-privileged HTTP control service, installed only on the dedicated GPU host (`gpu.mo-sys.de`), never alongside the other binaries. Switches the shared GPU between its normal chat role (`vllm-chat.service`) and image/video generation (`comfyui.service`, LTX-2.5), since both can't fit in VRAM at once -- `vllm-embed.service` is deliberately never touched, so search/indexing keeps working regardless of mode. Holds no DB connection and is outside this repo's own hexagonal ports/adapters graph entirely (no `internal/ports` interface on this side) -- it's the *server* a later `internal/adapters/httpgpumode` client adapter in `cmd/search` will call into over the private LAN, authenticated by a shared `X-Internal-Token` (`GPU_CONTROL_TOKEN`). See `packaging/gpu-control/README.md`.
