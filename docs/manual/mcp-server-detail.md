...
Command is the executable to spawn — an absolute path (e.g. /usr/bin/searchengine-mcp-web) or a name resolvable on this process's PATH. It runs directly, never through a shell, so pipes or globs won't work. Arguments are real argv elements, one per line: for mcp-sandbox, flags like -network (grant outbound network access, off by default — also unlocks a "packages" argument letting the model request pip/Go module installs, only possible with real network access), -system-packages (additionally let the model request OS-level packages — apt for run_python's Debian-based image, apk for run_go's Alpine-based one — only meaningful with -network, and a bigger privilege step than -network's own "packages" alone: installing them happens in a separate, short-lived container with a writable root filesystem, whose result is committed into a throwaway image that the actual code then runs from under the sandbox's completely normal lockdown, so installing packages and running the model's own code never share a container), -memory/-cpus/-pids-limit/-timeout (resource limits), -dns/-host-dns/-host-network for DNS and networking mode when -network is set, and -base-url (search-server's own base URL, defaults to http://127.0.0.1:8080, used to fetch a requested file_id's bytes -- independent of -network, see MCP Servers); for mcp-files, -base-url (defaults to http://127.0.0.1:8080). Every mcp-sandbox flag can also be set via a matching SE_SANDBOX_* environment variable (e.g. SE_SANDBOX_NETWORK, SE_SANDBOX_ALLOW_SYSTEM_PACKAGES) instead of Arguments, read the same way as any other environment-variable-configured setting — see [Environment variables](environment-variables.md) — though an explicit Arguments flag always wins over its env var. An admin filling in Command gets the same trust level as one configuring a crawl schedule or embedding endpoint — real trust, no extra sandboxing here, so only point it at binaries you actually trust.
...
Not where you describe what the server does — each tool's own name and description (via List tools above) already tells the model that. Prompt is for extra cross-tool steering no single tool's description can carry, e.g. "after searching, fetch results from different domains — not just the top-ranked ones — until you can validate the answer across independent sources; if a fetch is blocked, try a different domain rather than the same one again." A fixed count (e.g. "fetch the top 3") is the wrong shape for this: the top-ranked results can all share one domain, and if that domain blocks fetching, the model has nothing left to fall back to — a validation-based stopping condition survives that case, a fixed count doesn't. When non-empty and the server is active, it's injected as a system message after the chat endpoint's persistent prompt (Chat -> Settings), in addition to it. Leave empty if tool descriptions already say enough; most servers don't need one.
