...
| `packaging/gpu-control/gpu-control.env` | `/etc/searchengine/gpu-control.env` (on the GPU host only) | Auto (separate `.deb` conffile, edits preserved across upgrades) | Runtime config for `cmd/gpu-control` -- see [Environment variables](environment-variables.md). |
| `packaging/gpu-control/searchengine-gpu-control.service` | `/lib/systemd/system/searchengine-gpu-control.service` | Auto (separate `.deb`) | systemd unit for `cmd/gpu-control`. |
| `packaging/gpu-control/debian/{control,postinst,prerm}` | n/a -- become the separate `searchengine-gpu-control` `.deb`'s own metadata/maintainer scripts | Auto (build-time) | Package metadata, dependencies, install/removal behavior. Kept as its own package (`make deb-gpu-control`), never bundled into the main `.deb` -- see [packaging/gpu-control/README.md](https://github.com/M0WA/SE/blob/main/packaging/gpu-control/README.md). |
