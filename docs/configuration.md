...
| `server-command` | none, required for `stdio` | — | The executable to spawn -- never through a shell; an absolute path or one resolvable on the process's own `PATH`. The built-in web tools use `/usr/bin/searchengine-mcp-web`; the built-in `get_datetime` tool uses `/usr/bin/searchengine-mcp-datetime`. |
