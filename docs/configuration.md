...
16. **(Optional) Configure the built-in MCP tool servers.** The `.deb` already installs `/usr/bin/searchengine-mcp-web` (built from `cmd/mcp-web`), `/usr/bin/searchengine-mcp-datetime` (built from `cmd/mcp-datetime`), and `/usr/bin/searchengine-mcp-sandbox` (built from `cmd/mcp-sandbox`) — nothing to copy or `chmod`. None is a systemd service: search-server/admin-server spawn one on demand as a stdio subprocess (see `internal/adapters/mcpclient`) whenever an admin-configured `MCPServer` row points at it. Under Settings -> Chat -> MCP servers, add one row per binary: Transport=`stdio`, Command=`/usr/bin/searchengine-mcp-web` (tools: `web_search`/`web_fetch`), `/usr/bin/searchengine-mcp-datetime` (tool: `get_datetime`, current date/time, optionally in a given IANA timezone — the model calls this itself whenever it needs to know "now"; write the system/user prompt as "call get_datetime when you need the current date or time" rather than hardcoding a date), or `/usr/bin/searchengine-mcp-sandbox` (tools: `run_python`/`run_go`, each executing a model-supplied snippet inside a fresh, locked-down Docker container via `internal/adapters/dockersandbox` — dropped capabilities, a read-only root filesystem, and memory/CPU/process-count/wall-clock limits applied unconditionally; no network access unless this row's Args explicitly enable it), Enabled=checked. Each server's tools are exposed to the model as native tool-calling functions, which requires the chat endpoint below to actually support tool-calling (see the vLLM flags in the next step).
