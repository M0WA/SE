...
- **`cmd/mcp-datetime`** — a first-party MCP server exposing a single `get_datetime` tool (current date/time, optionally in a given IANA timezone) as a native tool-calling tool. Same operational model as `cmd/mcp-web` -- not a systemd service, spawned on demand as a `stdio` subprocess by `internal/adapters/mcpclient`. Replaces the old always-on `%c`/`strftime` prompt-placeholder mechanism (`application.expandPromptPlaceholders`): the model now asks for the current time only when it actually needs it, instead of the server injecting a formatted timestamp into every turn's system prompt whether or not it was needed.
